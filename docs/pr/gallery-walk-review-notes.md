...
- TicketsTonight is refocused into a gallery-walk discovery app with verified gallery inventory, route planning, route stop swapping, active-walk state, Taste Passport personalization, beta-preview tasks, source trust labels, and local persistence.
...
- Confirm the first screen shows the visual hero, first-run choices, beta task checklist, resume-walk path when applicable, verified inventory counts, personalized modules, and a freshness queue.
- On mobile web, confirm the bottom command bar exposes Tonight, Walk/Walking, For You, and beta progress without turning the page back into a dense dashboard.
...
- `app.json` now includes web name, short name, theme color, background color, and icon metadata so the project is closer to an installable mobile-web shell later.
...
- PWA readiness is metadata and mobile shell polish only; no service worker, offline cache, push notifications, or install prompt has been added.
